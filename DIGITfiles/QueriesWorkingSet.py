'''
QueriesWorkingSet is a class that stores the "best" data from a BlastOutput object. Queries are stored as Query objects
...
class QueriesWorkingSet:
    ...
    def __buildCompleteWorkingSet__(self, b73spec):
        # Fills QueriesWorkingSet object with all previously found query data. Used to compare sanger sequencing results
        ...
        for subdir, dirs, files in os.walk("DIGIToutput/FlankingSequences"):
            if subdir.find("JSON") != -1:
                for file in files:
                    if file.find("Working") != -1 and file.find("json") != -1 and file.find(b73spec) != -1:
    ...
    def __addToWorkingSet__(self, queryObj):
        # For adding a single Query object to workingSet as value (key is string Query.query). Used internally in class.
    ...
    def __getOriginalFlankingSequences__(self, flankseq):
        # Add flanking sequences to Query objects in working set from original fasta file that was blasted. Used in
        # SequencesFromBlast.
        flankseqDict = {}
    ...
    def __createQueryStructFromJson__(self, filename):
        # Build workingSet from JSON file. Used in GetPrimerIncidenceRate, GetPrimers, VerifyPrimers, and internally in
        # this class.
    ...
    def __updateQueryWithPrimer3Data__(self, outputDict, task):
        # Update Query objects in struct with parsed data from Primer3 initial output. Used in Utils.
    ...
    def __printToJson__(self, filepath, flankseq):
        # Write workingSet to a JSON file. Used in GetPrimers, SequencesFromBlast, and VerifyPrimers.
    ...
    def __printToCsv__(self, filename):
        ...
        with open(csvName, "w") as csvFile:
            csvFile.write(f"Query,Genome,BYearFamily,Chromosome,PercentIdentity,AlignmentLength,Mismatches,GapOpens,q" +
                          f"Start,qEnd,sStart,sEnd,eValue,BitScore,NumHitsForGenome,PercentDifferenceToNextHit,Strand" +
                          f",qStartStatus,PrimerNameLeft,PrimerSequenceLeft,PrimerLeftProductSize,PrimerPenaltyLeft,P" +
                          f"rimerLeftOrdered,PrimerLeftSangered,PrimerNameRight,PrimerSequenceRight,PrimerRightProduc" +
                          f"tSize,PrimerPenaltyRight,PrimerRightOrdered,PrimerRightSangered,PrimerPairProductSize,Pri" +
                          f"merPairPenalty\n")
    ...
    def __assignFilterFastaData__(self, seqData):
        # Assign sequence data returned from FilterFasta to the appropriate Query object. Used in Utils.
    ...
    def __setPrimerIDs__(self):
        ...
        for q in self.workingSet:
            ...
            if self.workingSet[q].primerNameLeft[0:2] != "mr":
                self.workingSet[q].primerNameLeft = f"mr{primerObject.numPrimers}" + self.workingSet[q].primerNameLeft
                primerObject.__append__(self.workingSet[q].primerNameLeft)
            if self.workingSet[q].primerNameRight[0:2] != "mr":
                self.workingSet[q].primerNameRight = f"mr{primerObject.numPrimers}" + self.workingSet[q].primerNameRight
    ...
    def __writeP3InputFile__(self, inputFilepath, task):
        # Initializes a Primer3Object for each query in queriesWorkingSet, and creates the input file that will be used
        ...
        with open(filename, "w+") as fafile:
            for q in self.workingSet:
                if self.workingSet[q].primerSequenceLeft != "FAIL":
                    fafile.write(f">{self.workingSet[q].primerNameLeft}_ogRefGen_{self.workingSet[q].genome}\n")
                    fafile.write(f"{self.workingSet[q].primerSequenceLeft}\n")
                if self.workingSet[q].primerSequenceRight != "FAIL":
                    fafile.write(f">{self.workingSet[q].primerNameRight}_ogRefGen_{self.workingSet[q].genome}\n")
                    fafile.write(f"{self.workingSet[q].primerSequenceRight}\n")


    def __createBadQueryStruct__(self):
        # Create a new QueriesWorkingSet object to hold failed primers to be saved in a different location.
        queriesWithBadPrimers = QueriesWorkingSet()
        for q in self.workingSet:
            if self.workingSet[q].primerSequenceRight == "FAIL" or self.workingSet[q].primerSequenceLeft == "FAIL":
    ...
    def __makePrimerDataFile__(self, filename):
        ...
        with open(filename, "w") as primerFile:
            primerFile.write(f"PrimerID,Allele,ReferenceGenome,PrimerSequence,MatchingDsGFPPrimer,ExpectedProductSize" +
                             f"WithDsPrimer,ExpectedWTProductSize,TM,PrimerPenaltyWithDSPrimer,PrimerPenaltyWT,BlastB" +
    ...
    def __havePrimersBeenOrderedOrTestedYet__(self):
        ...
        for q in self.workingSet:
            if self.workingSet[q].primerNameLeft in orderedPrimers:
                ...
                if self.workingSet[q].primerSequenceLeft != orderedPrimers[self.workingSet[q].primerNameLeft]:
            ...
            if self.workingSet[q].primerNameRight in orderedPrimers:
                ...
                if self.workingSet[q].primerSequenceRight != orderedPrimers[self.workingSet[q].primerNameRight]:
                    print(f"ERROR: {self.workingSet[q].primerNameRight} predicted primer " +
                          f"{self.workingSet[q].primerSequenceRight} does not match ordered primer " +
